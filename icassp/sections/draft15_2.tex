...
A reduced dense $2\times2$ repeats the same two $20{,}000$-step fine-tunes from AudioLDM-M-Full. The experimental fine-tunes are raw exports, so we also generate the raw weights of the released dense checkpoint on the same prompts. Raw and EMA dense baselines differ by at most about $0.02$ CLAP at either duration, allowing the dense fine-tunes to be interpreted without attributing a large change to the weight convention. This short-budget dense experiment is diagnostic rather than a substitute for the unavailable dense checkpoint after the released $10^{6}$-step recovery.
