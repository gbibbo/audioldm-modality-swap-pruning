...
The dense diagnostics sharpen the causal boundary. Raw and EMA dense baselines score similarly, so the severe drop in the short dense $2\times2$ is not a weight-convention artifact. The $20{,}000$-step recipe genuinely degrades an AudioCaps-trained dense model while improving the pruned model. This makes the dense experiment informative about the short adaptation recipe, but not a substitute for the unavailable dense checkpoint after the released $10^{6}$-step recovery. We therefore cannot determine whether the million-step operating-point dependence is specific to pruning, to fine-tuning more generally, or to another property of AudioLDM.
