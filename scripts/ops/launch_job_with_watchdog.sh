...
# Machine class via env MACHINE (default T4); e.g. MACHINE=L4 for a pre-approved 24 GB GPU contingency.
...
echo "launching $NAME from $SHA | machine $MACHINE | cap $MAXCOST cr / $MAXMIN min | $CMD"
lightning job run --name "$NAME" --machine "$MACHINE" --studio gabriel-allgd-deploy-model-devbox --teamspace general \
