...
* **M3A random-null + Gate-A statistic COMPLETE (M3-001), CPU, NO scientific result.** Resolves audit B1..B5. `research_pruning/diagnostics/random_masks.py` ports the reference pruning mechanic verbatim (`prune_with_indices` + 46-entry `LAYER_MAP`); **per-layer k comes from the pruned `[1,2,3,1]` target shapes, NOT from the pkl (permutations, not counts) nor the L1 ckpt**. 20 pre-registered random masks (seeds 20260818..37) with the same per-layer k as L1 (15 layers 960→192, 12 keep 576, 1 keeps 384; 10 176 ch/mask); L1+random materialise to **145.674M params**, strict-load, forward OK. Random-mask-set sha256 `90a05395…`. `matched_null.py`: linear OLS fit of R_mod~D_gen, `Delta_swap`, standardized residual, bootstrap over wavs+masks (**unit=wav, raises on repeats**). Tests **R1..R4** and **N1..N4 PASS** (N2 std-resid≈1.97, coverage≈0.96); Gate A can't pass by construction. **`l1_audioldm-m-full_p1.ckpt` never opened.** Evidence: `artifacts/m3_pilot/`.
