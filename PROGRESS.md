...
* **P0–P3 pruning-criteria MACHINERY COMPLETE (M3B-000), CPU, control-tested, NO scientific result.** `research_pruning/taylor/` and `research_pruning/paired_modality/` moved from skeletons to a working criteria package: channel-gate first-order Taylor saliency (`gates.py` `ChannelGate` g_c=1, `attach_gates`), `accumulate_taylor` (S_c=mean|g_c·∂L/∂g_c|), within-layer normalization (sum/max/l2), P0 L1 magnitude, P2 mean / P3 max combine, `assert_matched_budget` (§5: P1 2B == P2/P3 B+B), and `compute_criteria` orchestration sharing S_a/S_t. Tests `tests/research/test_taylor_saliency.py` **C1–C7 PASS** on control models. **The real prunable-layer→module mapping (28 L1 conv layers) and slot construction are NOT wired/frozen; NO saliency computed on the real L1/pruned checkpoint** (follows M3-000 precedent). **P1 is scientifically load-bearing — needs `/auditar` before any real use.** `git diff upstream-frozen -- audioldm_train/` still empty.
