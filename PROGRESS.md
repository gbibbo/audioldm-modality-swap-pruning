...
2. **M1 continuation:** finish CPU acceptance — F6 (inject + merge/unmerge on the real pruned `(1,2,3,1)` U-Net, record the parameter decomposition) and F8 (upstream integration hooks `setup_peft`/`build_peft_optimizer` + CPU tests, without patching `audioldm_train/`). Then M1 GPU acceptance (several hundred real steps, VRAM, sec/step, resume) — blocked on a GPU.
...
* CPU dummy-model tests **L1–L3, J1–J4, S1–S3 all PASS**. Recorded as **M1-005** in the ledger; `M1_CHECKLIST.md` updated. Remaining for M1 CPU acceptance: **F6** (real pruned U-Net) and **F8** (upstream integration hooks). No GPU work; `git diff upstream-frozen -- audioldm_train/` still empty.
